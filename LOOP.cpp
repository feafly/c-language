...
LRESULT CALLBACK WndProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
{
    ...
    /*case WM_PAINT:
    {
        ...
        for (iY = 0; iY < iYmax; iY++)
        {
            ...
            for (iX = 0; iX < iXmax; iX++)
            {
                ...
                Zx = Zy = Zx2 = Zy2 = 0.0;
                Interation = 0;
                while (Interation < IterationMax && ((Zx2 + Zy2) < ER2)
                {
                    double oldZx = Zx;
                    Zy = 2 * oldZx * Zy + Cy;
                    Zx = Zx2 - Zy2 + Cx;
    ...
    case WM_DESTROY:
        ...
        break;
    default:*/
    // 假设这是WM_PAINT的case（已经在switch(message)里）
    case WM_PAINT:
    {
        PAINTSTRUCT ps;
        HDC hdc = BeginPaint(hWnd, &ps);
        ...
        for (iY = 0; iY < iYmax; iY++)
        {
            ...
            for (iX = 0; iX < iXmax; iX++)
            {
                ...
                color = (Interation == IterationMax) ? RGB(0, 0, 0) : RGB(255, 255, 255);
                SetPixel(hdc, iX, iY, color);
            }
        }

        EndPaint(hWnd, &ps);
        break; // 闭合WM_PAINT的case
    }
